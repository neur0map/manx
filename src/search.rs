...
impl SearchEngine {
    ...
    pub async fn search(
        ...
        let docs = self.client.get_documentation(&library_id, Some(&optimized_query)).await?;
        
        // Step 4: Parse documentation into multiple results and rank them
        let results = self.parse_documentation_into_results(library, query, &docs, &search_terms)?;
        ...
        if let Ok(cache_manager) = crate::cache::CacheManager::new() {
            ...
            for (idx, result) in results.iter().enumerate() {
                if idx < sections.len() {
                    ...
                    let _ = cache_manager.set("snippets", &snippet_cache_key, &sections[idx]).await;
                }
            }
        }
    ...
    fn parse_documentation_into_results(
        ...
        for (idx, section) in sections.iter().enumerate() {
            ...
            if relevance > 0.1 { // Only include sections with reasonable relevance
                let title = self.extract_section_title(section)
    ...
    fn extract_section_excerpt(&self, section: &str) -> String {
        ...
        if let Some(desc_line) = section.lines().find(|line| line.starts_with("DESCRIPTION: ")) {
