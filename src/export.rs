use crate::client::{Documentation, SearchResult};
...
impl Exporter {
    pub fn export_search_results(results: &[SearchResult], path: &Path) -> Result<()> {
    ...
    pub fn export_documentation(doc: &Documentation, path: &Path) -> Result<()> {
        let format = ExportFormat::from_path(path);
    ...
    fn format_context7_content_for_markdown(content: &str) -> String {
        ...
        while i < lines.len() {
            ...
            if let Some(stripped) = line.strip_prefix("DESCRIPTION: ") {
                formatted.push_str(stripped);
                formatted.push_str("\n\n");
                i += 1;
                continue;
            }

            if let Some(stripped) = line.strip_prefix("SOURCE: ") {
