...
use docrawl::{crawl, Config, CrawlConfig};
use std::fs;
...
impl Indexer {
    ...
    pub async fn index_url_deep(
        ...
        match crawl(config).await {
            Ok(stats) => {
                log::info!(
                    "Docrawl completed successfully, processed {} pages",
                    stats.pages
                );
            }
            Err(e) => {
        ...
        log::info!(
            "Processing {} markdown files from docrawl",
            markdown_files.len()
        );
    ...
    pub(crate) async fn process_markdown_file(
        &self,
        md_file: &Path,
        base_url: &str,
    ) -> Result<Vec<DocumentChunk>> {
...
pub fn index_document(path: PathBuf, config: &RagConfig) -> Result<Vec<DocumentChunk>> {
    ...
    let code_extensions = [
        "js", "jsx", "ts", "tsx", "py", "rb", "php", "java", "scala", "kotlin", "rs", "go", "c",
        "cpp", "sh", "bash", "ps1",
    ];
...
fn extract_text(path: &Path, config: &RagConfig) -> Result<String> {
    ...
    match extension.as_str() {
        ...
        | "less" | "py" | "rb" | "php" | "java" | "scala" | "kotlin" | "groovy" | "c" | "cpp"
        | "cc" | "cxx" | "h" | "hpp" | "rs" | "go" | "zig" | "ml" | "mli" | "hs" | "elm"
        | "clj" | "cljs" | "erl" | "ex" | "exs" | "swift" | "m" | "mm" | "kt" | "dart" | "r"
        | "jl" | "lua" | "vim" | "el" | "sql" | "graphql" | "prisma" => {
            extract_code_text(path, config)
        }
        // Config files
        "json" | "yaml" | "yml" | "toml" | "xml" | "ini" | "properties" => {
            extract_config_text(path, config)
        }
...
fn validate_code_security(
    path: &Path,
    security_level: &crate::rag::CodeSecurityLevel,
) -> Result<()> {
    ...
    if is_potentially_obfuscated(&content) {
        match security_level {
            CodeSecurityLevel::Strict => {
                return Err(anyhow!(
                    "Code file rejected: Contains potentially obfuscated content"
                ));
            }
...
fn validate_shell_security(
    path: &Path,
    security_level: &crate::rag::CodeSecurityLevel,
) -> Result<()> {
    use crate::rag::CodeSecurityLevel;
    log::debug!(
        "Running enhanced security validation on shell script: {:?}",
        path
    );
    ...
    let compiled_patterns: Vec<regex::Regex> = dangerous_patterns
        .iter()
        ...
        for domain in &suspicious_domains {
            if url.contains(domain) {
                match security_level {
                    CodeSecurityLevel::Strict => {
                        return Err(anyhow!(
                            "Code rejected: Contains suspicious URL shortener: {}",
                            url
                        ));
                    }
                    CodeSecurityLevel::Moderate => {
    ...
    for pattern in &injection_patterns {
        if content_lower.contains(pattern) {
            match security_level {
                CodeSecurityLevel::Strict => {
                    return Err(anyhow!(
                        "Code rejected: Contains potential prompt injection pattern: {}",
                        pattern
                    ));
                }
                CodeSecurityLevel::Moderate => {
...
fn sanitize_code_content(content: &str) -> String {
    // Remove any inline secrets or API keys and preserve code structure
    mask_secrets(content)
}
